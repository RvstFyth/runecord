...
module.exports = {
    ...
    async create(user_id, quest_id) {
        return new Promise((resolve) => {
            db.query(
                `INSERT INTO ${this.table} (user_id, quest_id) VALUES (?,?)`,
                [user_id, quest_id],
                (err) => {
    ...
    async setCompleted(userID, questID) {
        return new Promise((resolve) => {
            db.query(
                `UPDATE ${this.table} SET completed = 1 WHERE user_id = ? AND quest_id = ?`,
                [userID, questID],
                (err) => {
                    if (err) console.log(err);
                    else resolve(true);
    ...
    async addAmount(userID, questID, val) {
        return new Promise((resolve) => {
            db.query(
                `UPDATE ${this.table} SET amount = amount + ? WHERE user_id = ? AND quest_id = ?`,
                [val, userID, questID],
                (err) => {
                    if (err) console.log(err);
                    else resolve(true);
