...
module.exports = {
    ...
    async create(userID, itemID, amount = 1, prefix = '') {
        return new Promise((resolve) => {
            db.query(
                `INSERT INTO ${this.table} (user_id, item_id, amount, prefix) VALUES (?,?,?,?)`,
                [userID, itemID, amount, prefix],
                (err, res) => {
    ...
    async getFor(userID, itemID, prefix = '') {
        return new Promise((resolve) => {
            db.query(
                `SELECT * FROM ${this.table} WHERE user_id = ? AND item_id = ? AND prefix = ?`,
                [userID, itemID, prefix],
                (err, rows) => {
                    if (err) console.log(err);
                    else resolve(rows);
                }
    ...
    async getTotalAmountFor(userID, itemID, prefix = '') {
        return new Promise((resolve) => {
            db.query(
                `SELECT SUM(amount) AS total FROM ${this.table} WHERE user_id = ? AND item_id = ? AND prefix = ?`,
                [userID, itemID, prefix],
                (err, rows) => {
                    if (err) console.log(err);
                    else
        ...
        if (item.stacks) {
            ...
            if (existingRecords && existingRecords.length)
                return this.setAmount(
                    ...
                    parseInt(existingRecords[0].amount) + amount
                );
    ...
    async addAmountFor(userID, itemID, amount, prefix = '') {
        return new Promise((resolve) => {
            db.query(
                `UPDATE ${this.table} SET amount = ? WHERE user_id = ? AND item_id = ? AND prefix = ?`,
                [amount, userID, itemID, prefix],
                (err) => {
                    if (err) console.log(err);
                    else resolve(true);
                }
    ...
    async deleteFor(userID, itemID, prefix = '') {
        return new Promise((resolve) => {
            db.query(
                `DELETE FROM ${this.table} WHERE user_id = ? AND item_id = ? AND prefix = ?`,
                [userID, itemID, prefix],
                (err) => {
                    if (err) console.log(err);
                    else resolve(true);
