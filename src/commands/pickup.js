...
module.exports = {
    ...
    async run(msg, args, data)
    {
        ...
        if(!args[0]) return msg.channel.send(`**${data.user.name}** what are you trying to pickup?!?`);
        ...
        if(!requested.length) return msg.channel.send(`**${data.user.name}** there is no ${input} to pickup..`);

        const itemRecord = await itemsModel.getForName(input);
        if(!itemRecord) return msg.channel.send(`**${data.user.name}** you can not pickup ${input}`);

        const inventoryCount = await inventoryModel.getOccupiedSlotCount(data.user.id);
        ...
        for(let i = 0; i < amount; i++) worldsHelper.removeObjectFromWorld(data.user.world, input, data.user.area, data.user.location)

        await inventoryModel.add(data.user.id, itemRecord.id, amount);
        return msg.channel.send(`**${data.user.name}** picked up ${amount} x ${itemRecord.name}`);
    }
};
