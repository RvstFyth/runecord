...
module.exports = {
    ...
    async run(msg, args, data)
    {
        ...
        if(amount < 1) return msg.channel.send(`**${data.user.name}** please specify a amount > 0`);

        if(!args[0]) return msg.channel.send(`**${data.user.name}** what are you trying to smelt?!?`);
        if(!itemMapping[args[0]]) return msg.channel.send(`**${data.user.name}** you can't smelt ${args[0]}`);
        
        const item = itemMapping[args[0]];
        const itemsCount = [];
        for(let i in item.items) {
            const itemRecord = await inventoryModel.getTotalAmountFor(data.user.id, i);
            itemsCount.push(Math.max(0, Math.floor(itemRecord / item.items[i])));
        }

        const maxPossible = Math.min(...itemsCount);
        if(!maxPossible) return msg.channel.send(`**${data.user.name}** you don't have the materials required...`);
        ...
        if(data.user.max_area === 'tutorial' && (parseInt(skillRecord.xp) + xpGain > skillsHelper.xpForLevel(3))) {
