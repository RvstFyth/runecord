...
module.exports = {
    ...
    {
        if(data.user) return msg.channel.send(`**${data.user.name}** is already registered...`);
        if(!args[0]) return msg.channel.send(`**${msg.author.username}** please specify a character name: \`${data.prefix}start [name]\``);
        ...
        const skills = [
            ...
            'mining', 'smithing', 'fishing', 'cooking', 'firemaking', 'woodcutting', 'agility', 'herblore', 'thieving',
