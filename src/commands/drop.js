    ...
    async run(msg, args, data) {
        if (!args[0])
            return msg.channel.send(
                `**${user.data.name}** what are you trying to drop?!?`
            );
        ...
        if (!isNaN(args[0])) {
            ...
            if (!amount)
                return msg.channel.send(
                    `**${data.user.name}** invalid amount provided..`
                );
            args = args.splice(1);
        } else amount = 1;
        ...
        if (!item)
            return msg.channel.send(
                `**${data.user.name}** there is no item named ${itemName}..`
            );
        ...
        const totalUserHas = valuesHelper.sumArray(
            userItems.map((x) => x.amount)
        );
        if (totalUserHas === 0)
            return msg.channel.send(
                `**${data.user.name}** you don't have any ${itemName}...`
            );
        ...
        return msg.channel.send(
            `**${data.user.name}** dropped ${amount} x ${item.name}`
        );
    },
};
