...
module.exports = {
    ...
    async run(msg, args, data)
    {
        ...
        if(!locationDetails.commands || (locationDetails.commands && !locationDetails.commands.fish)) return msg.channel.send(`**${data.user.name}** you can't fish here?!?`);

        if(!args[0]) return msg.channel.send(`**${data.user.name}** what are you trying to fish?!?`);
        if(!locationDetails.commands.fish[args[0]]) return msg.channel.send(`**${data.user.name}** you can't fish for ${args[0]} here..`);
        ...
        return msg.channel.send(`**${data.user.name}** caught a ${item.name} ${reward.xp > 0 ? `and ${reward.xp}xp!` : ''}`);
