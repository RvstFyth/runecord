    ...
    async run(msg, args, data) {
        ...
        if (!input) {
            return msg.channel.send(
                `**${data.user.name}** who are you trying to pickpocket?!?`
            );
        }

        const mobsAtLocation = worldsHelper.getMobs(
            data.user.world,
            data.user.area,
            data.user.location
        );
        if (!Object.keys(mobsAtLocation).length)
            return msg.channel.send(
        ...
        if (!mobsAtLocation[input])
            return msg.channel.send(
                `**${data.user.name}** there is no ${input} here...`
            );
        if (!mobsAtLocation[input] || !filteredMobs.length)
            return msg.channel.send(
                `**${data.user.name}** there is no ${input} here...`
            );
        ...
        if (!mob.pickpocket)
            return msg.channel.send(
                `**${data.user.name}** you can't pickpocket ${input}...`
            );

        if (data.char.skills.thieving.level < mob.pickpocket.level)
            return msg.channel.send(
                `**${data.user.name}** you need thieving level ${mob.pickpocket.level} for this...`
            );

        const seconds = random.number(15, 30);

        if (random.number(1, 100) <= mob.pickpocket.baseChance) {
            const xpAdded = await data.char.skills.thieving.addXp(
                mob.pickpocket.xp
            );
            const loot = [];
            if (mob.pickpocket.loot.gold) {
                const gold = random.number(
                    mob.pickpocket.loot.gold.min,
                    mob.pickpocket.loot.gold.max
                );
            ...
            return msg.channel.send(
                `**${data.user.name}** you failed to pickpocket ${input}..`
            );
    },
};
